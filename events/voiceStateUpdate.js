    ...
    switch (stateChange.type) {
        case "JOIN":
            if (stateChange.members.size === 1 && player.paused) {
                let emb = new MessageEmbed()
                    .setAuthor(`Снимаю очередь с паузы`, client.botconfig.IconURL)
                    .setColor("RANDOM")
                    .setDescription(`Снимаю паузу, потому что вы сука меня кинули одного`);
        ...
        case "LEAVE":
            if (stateChange.members.size === 0 && !player.paused && player.playing) {
                ...
                let emb = new MessageEmbed()
                    .setAuthor(`Поставил на паузу`, client.botconfig.IconURL)
                    .setColor(client.botconfig.EmbedColor)
                    .setDescription(`Я поставил паузу, потому что все сука ушли`);
                await client.channels.cache.get(player.textChannel).send(emb);
            }
            break;
    }
