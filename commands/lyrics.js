...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] && !player) return client.sendTime(message.channel, "❌ | **Сейчас не играет ничего**");
    ...
    if (!lyrics) return client.sendTime(message.channel, `**Не нашел текста к -** \`${SongTitle}\``);
    lyrics = lyrics.split("\n"); //spliting into lines
  ...
  SlashCommand: {
    options: [
      {
        ...
        value: "song",
        type: 3,
        description: "Введи название песни для поиска",
        required: false,
      },
    ],
    /**
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!interaction.data.options && !player) return client.sendTime(interaction, "❌ | **Сейчас не играет ничего**");
      ...
      if (lyrics.length === 0)
        return client.sendTime(interaction, `**Не нашел текста к -** \`${SongTitle}\``);
      lyrics = lyrics.split("\n"); //spliting into lines
      let SplitedLyrics = _.chunk(lyrics, 40); //45 lines each page

      let Pages = SplitedLyrics.map((ly) => {
        let em = new MessageEmbed()
          .setAuthor(`Текст к: ${SongTitle}`, client.botconfig.IconURL)
          .setColor(client.botconfig.EmbedColor)
          .setDescription(ly.join("\n"));
