...
module.exports = {
  name: "search",
  description: "Показывает список песен, на основе фильтра",
  usage: "[песня]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Использование - **\`${GuildDB.prefix}search [критерий]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      return client.sendTime(
        message.channel,
        "Не нашел ничего по " + SearchString
      );
    else {
      ...
      let Pages = songs.map((songz) => {
        ...
        let em = new MessageEmbed()
          .setAuthor("Результаты поиска по " + SearchString, client.botconfig.IconURL)
      ...
      let msg = await message.channel.send(
        "**Введи кол-во песен для воспроизведения. Истекает через `30 секкунд`.**"
      );
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(message.channel, "Пришли правильный ID песни");
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song) return client.sendTime(message.channel, "Ничего не нашел по данному ID");
    ...
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Введи имя или url песни",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "NO_MATCHES":
            if (!player.queue.current) player.destroy();
            return client.sendTime(interaction, ":x: | **Нет результатов**");
          case "TRACK_LOADED":
            player.queue.add(TrackUtils.build(Searched.tracks[0], member.user));
            if (!player.playing && !player.paused && !player.queue.length)
              player.play();
            return client.sendTime(
              interaction, `**Добавлено в очередь:** \`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri}}\`.`
            );
        ...
        try {
          ...
          return client.sendTime(
            interaction, `:x: | **Ошибка при поиске:** ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
          ...
          case "TRACK_LOADED":
            ...
            if (!player.playing && !player.paused && !player.queue.length)
              player.play();
            return client.sendTime(
              interaction, `**Добавлено в очередь:** \`[${res.tracks[0].title}](${res.tracks[0].uri})\`.`
            );
          case "PLAYLIST_LOADED":
          ...
          case "SEARCH_RESULT":
            ...
            const resultss = new MessageEmbed()
              .setDescription(
                `${results}\n\n\t**Введи кол-во песен для воспроизведения**\n`
              )
              .setColor(client.botconfig.EmbedColor)
              .setAuthor(`Результаты поиска для ${search}`, client.botconfig.IconURL);
            ...
            if (first.toLowerCase() === "cancel") {
              if (!player.queue.current) player.destroy();
              return awaitchannel.send("Отменил поиск");
            }

            const index = Number(first) - 1;
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Число меньше которое ты написал меньше или больше результата поиска. Использование - \`(1-${max})\``
              );
            ...
            if (!player.playing && !player.paused && !player.queue.length) {
              ...
              if (player.queue.totalSize > 1) SongAddedEmbed.addField("Место в очереди", `${player.queue.size - 0}`, true);
