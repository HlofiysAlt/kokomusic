...
module.exports = {
    name: "seek",
    description: "Перейти к позиции в песне",
    usage: "<время с/м/х>",
    permissions: {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!player.queue.current.isSeekable) return client.sendTime(message.channel, "❌ | **Не могу найти эту песню**");
        let SeekTo = client.ParseHumanTime(args.join(" "));
        if (!SeekTo) return client.sendTime(message.channel, `**Использование - **\`${GuildDB.prefix}seek <число с/м/ч>\` \n**Пример - **\`${GuildDB.prefix}seek 2м 10с\``);
